...
class NamingWorkflowFragment : ToolbarPreferenceFragment(R.xml.naming_workflow_preferences, R.string.root_naming_workflow), MetadataManager, CoroutineScope by MainScope() {

    private val TAG = "NamingWorkflowFragment"
    ...
    private fun setupNamingPreferences() {
        ...
        findPreference<Preference>(mKeyUtil.nameCreatePatternKey)?.setOnPreferenceClickListener {
            findNavController().navigate(R.id.action_namingWorkflowFragment_to_patternFragment)
            true
        }
    }
    ...
    private fun setupMetadataPreferences() {
        try {
            ...
            metadataPref?.setOnPreferenceClickListener {
                context?.let { ctx -> MetadataCreatorDialog(ctx, this@NamingWorkflowFragment).show() }
                true
            }

            defaultsPref?.setOnPreferenceClickListener {
                findNavController().navigate(R.id.action_namingWorkflowFragment_to_metadataList)
                true
            }
        } catch (e: Exception) {
